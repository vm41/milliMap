...
class Pix2PixHDModel(BaseModel): # Inherited from the BaseModel class, which mainly contains save and load model functions
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            # grab the edge loss function
            if not opt.no_prior_loss:
                self.criterionPrior = networks.PriorLoss(self.gpu_ids, opt.detector_type)

            # Names so we can breakout loss
            self.loss_names = self.loss_filter('G_GAN','G_GAN_Feat','G_VGG', 'G_Prior',
                                               'D_real', 'D_fake')
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        # Map prior loss
        loss_G_Prior = 0
        if not self.opt.no_prior_loss:
            loss_G_Prior = self.criterionPrior(fake_image, real_image) * self.opt.lambda_prior
        
        # Only return the fake_B image if necessary to save BW
        return [ self.loss_filter( loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_G_Prior,
